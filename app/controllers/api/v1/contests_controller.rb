class Api::V1::ContestsController < ApplicationController
  ...
  before_action :set_contest, only: [:show, :problems]
  ...
  def problems
    if authorized_user?
      render json: @contest.problems, each_serializer: ProblemSerializer, current_user: current_user, contest_id: @contest.id
    else
      render json: { error: "Not authorized" }, status: :forbidden
    end
  end
